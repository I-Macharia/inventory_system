...
async def login(credentials: LoginRequest, db: Session = Depends(get_db)):
    ...
    # First check fake users (for admin)
    fake_user = FAKE_USERS.get(credentials.username)
    if fake_user and fake_user["password"] == credentials.password:
...
def approve_request(request_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    # Create real user with email as username
    temporary_password = "TempPass123!"  # In production, generate a random password
    new_user = User(
        email=req.email,
    ...
    # For now, return it in the response (NEVER do this in production!)
    return {
